...
class SubPolicy(object):
    ...
    def _image_ops(self, operation, img, magnitude, fillcolor):
        if operation == 'shearX':
            ...
            img = img.transform(img.size, Image.AFFINE, (1, 0, magnitude * img.size[0] * random.choice([-1, 1]), 0, 1, 0),
                            fillcolor=fillcolor)
        elif operation == "translateY":
            img = img.transform(img.size, Image.AFFINE, (1, 0, 0, 0, 1, magnitude * img.size[1] * random.choice([-1, 1])),
                            fillcolor=fillcolor)
