...
from learnning_padding import LearningPaddingByConvolution
...
def conv3x3(in_planes, out_planes, stride=1):
	...
	"""
	return nn.Sequential(LearningPaddingByConvolution(in_planes),
...
class Bottleneck(nn.Module):
	...
	def __init__(self, inplanes, planes, stride=1, downsample=None, p_shakedrop=1.0):
		...
		self.bn2 = nn.BatchNorm2d(planes)
		self.conv2 = nn.Sequential(LearningPaddingByConvolution(planes, bn=ShakeDrop(p_shakedrop)),
		...
		self.stride = stride
		# self.shake_drop = ShakeDrop(p_shakedrop)
		self.shake_drop = ShakeDrop(0)
		self.drop_path = DropPath(p_shakedrop)
	...
	def forward(self, x):
		...
		out = self.shake_drop(self.drop_path(out))
...
class PyramidNet(nn.Module):
	def __init__(self, dataset='cifar10',
		...
		if self.dataset in ['cifar10', 'cifar100', 'svhn']:
			...
			# self.ps_shakedrop = [0 for i in range(3 * n)]

			self.input_featuremap_dim = self.inplanes
			self.conv1 = nn.Sequential(LearningPaddingByConvolution(3),
				nn.Conv2d(3, self.input_featuremap_dim, kernel_size=3, stride=1, padding=0, bias=False))
			...
			self.input_featuremap_dim = self.inplanes
			self.conv1 = nn.Sequential(LearningPaddingA_AllStride(3, 3),
				nn.Conv2d(3, self.input_featuremap_dim, kernel_size=7, stride=2, padding=0, bias=False))
			self.bn1 = nn.BatchNorm2d(self.input_featuremap_dim)
			self.relu = nn.ReLU(inplace=_inplace_flag)
			self.maxpool = nn.Sequential(LearningPaddingA_AllStride(self.input_featuremap_dim),
