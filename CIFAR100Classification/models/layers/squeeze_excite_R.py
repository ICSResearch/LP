...
from .AS import StdGate, StdMaskLinear

class SE_RModule(nn.Module):
    ...
    def __init__(
        ...
        super(SE_RModule, self).__init__()
        self.add_maxpool = add_maxpool
        if not rd_channels:
            rd_channels = make_divisible(channels * rd_ratio, rd_divisor, round_limit=0.)
        self.bn = norm_layer(rd_channels) if norm_layer else nn.Identity()
        self.fc = StdGate(channels, rd_channels, act=False, bias=False, shortcut=True)
    ...
    def forward(self, x):
        ...
        x_se = self.fc(x_se).view(x.size(0), x.size(1), 1, 1)
        x_se = self.bn(x_se)
        # x_se = self.fc2(x_se)
        return x * self.gate(x_se)


SqueezeExcite = SE_RModule  # alias
