...
class ConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1,
        ...
        layers.append(PartialConv2d(in_channels=in_planes, out_channels=out_planes,
...
class SEBlock(nn.Module):
    ...
    def __init__(self, in_planes, reduced_dim, memory_efficient=False):
        ...
        self.channel_se = nn.Sequential(OrderedDict([
            ("linear1", PartialConv2d(in_planes, reduced_dim, kernel_size=1, stride=1, padding=0, bias=True)),
            ("act", Swish(inplace=True) if not memory_efficient else MemoryEfficientSwish()),
            ("linear2", PartialConv2d(reduced_dim, in_planes, kernel_size=1, stride=1, padding=0, bias=True))
        ]))
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes,
        ...
        self.point_wise = nn.Sequential(OrderedDict([
            ("conv", PartialConv2d(in_channels=hidden_dim,
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, arch="bo", num_classes=100, dataset='cifar10', split_factor=1,
