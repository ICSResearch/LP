...
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 100, 'input_size': (3, 32, 32), 'pool_size': (1, 1),
...
class RegNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, output_stride=32, global_pool='avg', drop_rate=0.,
        ...
        self.stem = ConvBnAct(in_chans, stem_width, 3, stride=1)
...
def regnetx_004(pretrained=False, pt=None, **kwargs):
    """RegNetX-400MF"""
    net = _create_regnet('regnetx_004', pretrained, **kwargs)
    # net.stem.conv.stride = (1, 1)
    # net.s2.b1.conv2.conv.stride = (1, 1)
    # net.s2.b1.downsample.conv.stride = (1, 1)
    if pt in ('LPA', 'LPC', 'part'):
        up = Upgrade(net, pt)
        net = up.new_net
...
def regnety_004(pretrained=False, pt=None, **kwargs):
    """RegNetY-400MF"""
    net = _create_regnet('regnety_004', pretrained, **kwargs)
    net.stem.conv.stride = (1, 1)
    net.s1.b1.conv2.conv.stride = (1, 1)
    net.s1.b1.downsample.conv.stride = (1, 1)
    net.s2.b1.conv2.conv.dilation = (2, 2)
    net.s2.b1.conv2.conv.padding = (2, 2)
    net.s3.b1.conv2.conv.dilation = (2, 2)
    net.s3.b1.conv2.conv.padding = (2, 2)
    net.s4.b1.conv2.conv.dilation = (2, 2)
    net.s4.b1.conv2.conv.padding = (2, 2)
